package base.graph;

public class DijkstraNormal {

	public static void main(String[] args) {
		GraphDijkstra g = new GraphDijkstra(8);
		g.input(1, 2, 3);
		g.input(1, 5, 4);
		g.input(1, 4, 4);
		g.input(2, 3, 2);
		g.input(3, 4, 1);
		g.input(4, 5, 2);
		g.input(5, 6, 4);
		g.input(4, 7, 6);
		g.input(7, 6, 3);
		g.input(3, 8, 3);
		g.input(6, 8, 2);
		g.printMap();
		g.dijkstra(1);
	}

}

class GraphDijkstra {
	private int n; // 노드들의 수
	private int adj[][]; // 노드들간의 가중치 저장할 변수

	public GraphDijkstra(int n) {
		this.n = n;
		adj = new int[n + 1][n + 1];

	}

	public void printMap() {
		System.out.println("----------------  adj  ---------------");
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				System.out.print(adj[i][j] + " ");
			}
			System.out.println();
		}
		System.out.println("--------------------------------------");
	}

	public void input(int i, int j, int w) {
		adj[i][j] = w;
		adj[j][i] = w;
	}

	public void dijkstra(int v) {
		int distance[] = new int[n + 1]; // 최단 거리를 저장할 변수
		boolean[] check = new boolean[n + 1]; // 해당 노드를 방문했는지 체크할 변수

		// distance값 초기화.
		for (int i = 1; i < n + 1; i++) {
			distance[i] = Integer.MAX_VALUE;
		}

		// 시작노드값 초기화.
		distance[v] = 0;
		check[v] = true;

		// 연결노드 distance갱신
		for (int i = 1; i < n + 1; i++) {
			if (!check[i] && adj[v][i] != 0) {
				distance[i] = adj[v][i];
			}
		}

		for (int a = 0; a < n - 1; a++) {
			// 원래는 모든 노드가 true될때까지 인데
			// 노드가 n개 있을 때 다익스트라를 위해서 반복수는 n-1번이면 된다.
			// 원하지 않으면 각각의 노드가 모두 true인지 확인하는 식으로 구현해도 된다.
			int min = Integer.MAX_VALUE;
			int min_index = -1;

			// 최소값 찾기
			// 방문하지 않은 노드 && 거리가 갱신된 적이 있는 간선
			// -> 현재까지 방문하지 않은 노드 중 최소 간선으로 갈 수 있는 노드를 정한다
			for (int i = 1; i < n + 1; i++) {
				if (!check[i] && distance[i] != Integer.MAX_VALUE) {
					if (distance[i] < min) {
						min = distance[i];
						min_index = i;
					}
				}
			}
			// 채택된 노드에 visited check한다
			check[min_index] = true;
			
			// 채택된 노드에서 다른 간선으로 가는 비용이 기존 비용보다 낮을 경우 대체(갱신)한다.
			for (int i = 1; i < n + 1; i++) {
				if (!check[i] && adj[min_index][i] != 0) {
					if (distance[i] > distance[min_index] + adj[min_index][i]) {
						distance[i] = distance[min_index] + adj[min_index][i];
					}
				}
			}
		}

		// 결과값 출력
		for (int i = 1; i < n + 1; i++) {
			System.out.print(distance[i] + " ");
		}
		System.out.println("");

	}
}
